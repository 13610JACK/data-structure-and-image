...
int main()
{
	...
	double t; //timing variable
			  ...
			  //load training image
	Mat object = imread("I:/国画下载/0.jpg", CV_LOAD_IMAGE_GRAYSCALE);
	if (!object.data) {
	...
	if (testing)
	{
		...
		int minH;
		std::ofstream file;
		file.open("I:/国画下载/1.jpg", std::ofstream::out);
		for (int i = 0; i<20; i++)
		{
			minH = minHessVector[i];
			t = (double)getTickCount();
			//SurfFeatureDetector detector(minH);
			Ptr<SURF> detector = SURF::create(minH);
		...
		imwrite("I:/国画下载/2.jpg", interestPointObject);
		waitKey(0);
	}
